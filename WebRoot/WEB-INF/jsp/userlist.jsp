	...
	<table class="providerTable" cellpadding="0" cellspacing="0">
		...
		<c:forEach var="user" items="${userList }" varStatus="status">
			<tr>
				...
				<td><span> <c:if test="${user.gender==1}">女</c:if> <c:if
							test="${user.gender==2}">男</c:if>
				</span></td>
	...
	<div class="providerAdd">
		<div>
			<label for="userCode">用户编码：</label> <input type="text"
				name="v_userCode" id="v_userCode" value="" readonly="readonly">
		</div>
		<div>
			<label for="userName">用户名称：</label> <input type="text"
				id="v_userName" name="v_userName" value="" readonly="readonly">
		</div>
		<div>
			<label>用户性别：</label> <input type="text" id="v_gender" name="v_gender"
				readonly="readonly" value="">
		</div>
		<div>
			<label for="birthday">出生日期：</label> <input type="text" Class="Wdate"
				id="v_birthday" name="v_birthday" readonly="readonly"
				onclick="WdatePicker();">
		</div>
		<div>
			<label for="phone">用户电话：</label> <input type="text" name="v_phone"
				id="v_phone" value="" readonly="readonly">
		</div>
		<div>
			<label>用户角色：</label> <input type="text" id="v_userRoleName"
				name="v_userRoleName" readonly="readonly">
		</div>
		<div>
			<label for="address">用户地址：</label> <input name="v_address"
				id="v_address" value="" readonly="readonly">
		</div>
		</div>
</div>
</section>
